...
from _shared_code import get_images
...
def load_image_train(img_path, segmentation_mask_path):
    img_data = tf.io.read_file(img_path)
    input_image = tf.io.decode_bmp(img_data)

    segm_data = tf.io.read_file(segmentation_mask_path)
    input_mask = tf.io.decode_bmp(segm_data)
...
def load_image_test(img_path, segmentation_mask_path):
    img_data = tf.io.read_file(img_path)
    input_image = tf.io.decode_bmp(img_data)
...
train_dataset = training_dataset.map(load_image_train, num_parallel_calls=tf.data.AUTOTUNE)
test_dataset = val_dataset.map(load_image_test, num_parallel_calls=tf.data.AUTOTUNE)

BATCH_SIZE = 4
BUFFER_SIZE = 1000
train_batches = train_dataset.cache().shuffle(BUFFER_SIZE).batch(BATCH_SIZE)        # removed  .repeat()
